...
import { FiCheckCircle, FiClock, FiExternalLink, FiInbox, FiSearch, FiTrash2 } from "react-icons/fi";
...
export function SellRequestsClient() {
  ...
  const reset = () => { setQuery(""); setStatus(""); setType(""); setRange("30"); setFrom(""); setTo(""); setPage(1); };
  ...
  return <AdminShell>
    ...
    <div className="admin-toolbar request-toolbar">
      ...
      <select aria-label="بازه ارسال درخواست" value={range} onChange={(event) => { setRange(event.target.value); setPage(1); }}><option value="7">۷ روز اخیر</option><option value="30">۳۰ روز اخیر</option><option value="90">۹۰ روز اخیر</option><option value="all">همه زمان‌ها</option><option value="custom">بازه سفارشی</option></select>
