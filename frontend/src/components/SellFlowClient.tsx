...
import { FiArrowLeft, FiArrowRight, FiCamera, FiCheck, FiCheckCircle, FiClock, FiCopy, FiInfo, FiMap, FiPlus, FiShield, FiTrash2, FiX } from "react-icons/fi";
...
function StepHeader({ heading, fa, step, title, text }: { heading: React.RefObject<HTMLHeadingElement | null>; fa: boolean; step: number; title: string; text: string }) { return <header className="sell-step-head"><small>{fa ? `مرحله ${step} از ۳` : `Step ${step} of 3`}</small><h2 ref={heading} tabIndex={-1}>{title}</h2><p>{text}</p></header>; }
